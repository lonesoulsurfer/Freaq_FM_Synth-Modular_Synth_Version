...
// Mozzi 2.0 Configuration - MUST be before including Mozzi.h
...
#define DISPLAY_SETTING_INTENSITY 1
...
// MOZZI variables
// MOZZI_CONTROL_RATE now defined at top of file (before Mozzi.h include)
...
void setup() 
{
  ...
  startMozzi(); // Mozzi 2.0: no parameters
}
 ...
 *----------------------------------------------------------------------------------------------------------
 */
AudioOutput_t updateAudio()  // Mozzi 2.0: returns AudioOutput_t
{
  long mixed = (long)voices[0]->updateAudio() + (long)voices[1]->updateAudio();
  ...
  return MonoOutput::fromNBit(9, mixed);
}
